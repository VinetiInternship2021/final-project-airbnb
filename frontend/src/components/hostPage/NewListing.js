...
function getCoordinates(myAddress) {
  const encodedAddress = encodeURI(myAddress);

  return fetch(
    `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodedAddress}.json?access_token=pk.eyJ1Ijoia2Fybzk4IiwiYSI6ImNrb2JpNGFxOTA4Nmoyb24xenlscXZ4bmUifQ.3OiaCJK72mS4W2lnrPKPTg`
  ).then((res) => res.json());
}
...
function NewListing({ currentUser }) {
  ...
  const createNewProperty = async (e) => {
    ...
    const coordinateResult = await getCoordinates(form.address);

    const obj_coordinates = {
      latitude: coordinateResult.features[0].center[1].toString(),
      longitude: coordinateResult.features[0].center[0].toString(),
    };

    const newProp = await reqCreateToken(
      "create_property",
      { ...form, ...obj_coordinates },
      currentUser[0].token
    ); //create property
    ...
    success("Property succesfully created");
